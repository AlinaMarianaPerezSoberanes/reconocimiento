...
unsigned char *abrirBMP(char *filename, bmpInfoHeader *bInfoHeader)
{
	...
	if( f  == NULL )
	{
  	...
  	if( type != 0x4D42 )        /* Comprobamos el formato de imagen BMP*/
    	{
  	...
  	/* Leemos la cabecera de fichero completa con la estructura de la cabecera*/
  	fread( &header, sizeof(bmpFileHeader), 1, f );

  	/* Leemos la cabecera de información completa segunda estructura */
